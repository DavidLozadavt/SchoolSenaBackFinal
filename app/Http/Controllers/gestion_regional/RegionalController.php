...
class RegionalController extends Controller
{
    public function store(Request $request)
    {
        ...
        try {
            $nuevaRegional = DB::transaction(function () use ($request, &$rutaDocumento) {
                ...
                // 1️⃣ Crear empresa
                $nuevaRegional = new Company();
                $nuevaRegional->razonSocial = $request->razonSocial;
                $nuevaRegional->nit = $request->nit;
                $nuevaRegional->representanteLegal = $request->representanteLegal;
                $nuevaRegional->direccion = $request->direccion;
                $nuevaRegional->email = $request->email;
                $nuevaRegional->rutaLogo = $rutaDocumento ?? Company::RUTA_LOGO_DEFAULT;
                $nuevaRegional->digitoVerificacion = $request->digitoVerificacion;
                $nuevaRegional->idCiudad = $request->idCiudad;
                $nuevaRegional->save();


                // 2️⃣ Crear usuario
                $emailRegional = 'administrador' .
                    Str::of($nuevaRegional->razonSocial)
                    ->ascii()        // quita tildes
                    ->lower()        // minúsculas
                ...
                $activeUser->company_id = $nuevaRegional->id;
            ...
            if ($rutaDocumento) {
                Storage::disk('public')->delete(
                    str_replace('/storage/', '', $rutaDocumento)
                );
            }
            return response()->json([
                'status' => 'error',
